...
class Ms_Category_Model extends CI_Model
{
	...
	**/
	...
	// function get_wakunwis($id)
	// {
	// 	$this->db->order_by('post_kunjungan_sec','asc');
	// 	$result = $this->db->get('posts');
	// 	$dd[''] = 'waktu kunjungan'
	// 	if ($result->num_rows() > 0){
	// 		foreach ($result->result() as $row){
	// 			$dd[$row->post_id] = $row->post_kunjungan_sec;
	// 		}
	// 	}
	// 	return $dd;
	// }
	function get_all_wakunwis()
	{
	return $this->db->query("SELECT post_kunjungan_sec FROM posts");
	}

	function get_waktu_id($id)
	{
		return $this->db->query("SELECT post_kunjungan_sec FROM posts
									WHERE post_id = '$id'");
	}
